...
function generateMachines(): Machine[] {
  ...
  Object.entries(CITIES_AND_DORMS).forEach(([city, dorms]) => {
    dorms.forEach((dorm) => {
      const machineCount = Math.floor(Math.random() * 20) + 15;
      ...
      for (let i = 0; i < machineCount; i++) {
        ...
        const statusIndex = (machineCounter - 1) % 4;
...
function generateUsers(): User[] {
  ...
  Object.entries(CITIES_AND_DORMS).forEach(([city, dorms]) => {
    dorms.forEach((dorm) => {
      const userCount = Math.floor(Math.random() * 60) + 40;
      ...
      for (let i = 0; i < userCount; i++) {
        ...
        users.push({
          ...
          phone: `+49 ${Math.floor(Math.random() * 900) + 100} ${Math.floor(
            Math.random() * 9000,
          ) + 1000}${Math.floor(Math.random() * 9000) + 1000}`,
          status,
          city,
          dorm,
...
function generateReservations(machines: Machine[], users: User[]): Reservation[] {
...
function generateProfileRequests(users: User[]): ProfileRequest[] {
  ...
  for (let i = 0; i < 150; i++) {
    ...
    switch (requestType) {
      ...
      case 'dorm_change':
        ...
        newValue = CITIES_AND_DORMS[user.city as keyof typeof CITIES_AND_DORMS][
          Math.floor(Math.random() * CITIES_AND_DORMS[user.city as keyof typeof CITIES_AND_DORMS].length)
        ];
        break;
      case 'contact_update':
        currentValue = user.phone;
        newValue = `+49 ${Math.floor(Math.random() * 900) + 100} ${Math.floor(
          Math.random() * 9000,
        ) + 1000}${Math.floor(Math.random() * 9000) + 1000}`;
        break;
    }
    ...
    requests.push({
      ...
      currentValue,
      newValue,
      reason: ['Moving to different location', 'Closer to classes', 'Better facilities', 'Personal reasons'][
        Math.floor(Math.random() * 4)
      ],
...
function generateUserQueries(users: User[]): UserQuery[] {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    queries.push({
      ...
      adminResponse:
        status === 'resolved' ? 'Thank you for reporting this issue. It has been resolved.' : undefined,
      city: user.city,
      dorm: user.dorm,
    });
  }
...
export const mockReservations = generateReservations(mockMachines, mockUsers);
export const mockProfileRequests = generateProfileRequests(mockUsers);
export const mockUserQueries = generateUserQueries(mockUsers);
